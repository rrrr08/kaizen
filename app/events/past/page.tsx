...
export default function PastEvents() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'All' | 'Workshop' | 'Game Night' | 'Other'>('All');
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 bg-[#FFFDF5]">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="mb-20 border-b-4 border-black pb-12">
          <div className="flex flex-col items-start">
            ...
            <h1 className="font-header tracking-tighter text-[#2D3436] flex flex-col items-start leading-none">
              <span className="text-3xl md:text-4xl font-black uppercase mb-1">PAST</span>
            ...
            <p className="text-[#2D3436]/60 font-black text-sm md:text-xl mt-6 uppercase tracking-wider flex items-center gap-2">
              <span className="w-2 h-2 bg-[#6C5CE7] rounded-full opacity-50"></span>
        ...
        <div className="flex flex-nowrap overflow-x-auto no-scrollbar gap-4 mb-12 pb-4 -mx-6 px-6 md:mx-0 md:px-0 md:flex-wrap">
          {['All', 'Workshop', 'Game Night', 'Other'].map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl font-black text-xs tracking-widest uppercase transition-all border-2 border-black ${selectedCategory === cat
                ? 'bg-[#FFD93D] text-black shadow-[4px_4px_0px_#000]'
                : 'bg-white text-black/40 hover:text-black hover:border-black'
                }`}
            >
        ...
        {/* Calendar Section */}
        <EventCalendar events={events.filter(e => selectedCategory === 'All' || (e.category || 'Other') === selectedCategory)} mode="past" />
