...
export default function GameSettingsPage() {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {Object.entries(settings).map(([gameId, cfg]: any) => (
        <div key={gameId} className="mb-6 border p-4 rounded">
          ...
          <form onSubmit={e => { e.preventDefault(); handleSave(gameId, cfg); }}>
            <label>Base Points: <input type="number" defaultValue={cfg.basePoints} onChange={e => cfg.basePoints = +e.target.value} /></label><br />
            <label>Retry Penalty: <input type="number" defaultValue={cfg.retryPenalty} onChange={e => cfg.retryPenalty = +e.target.value} /></label><br />
              ...
              <div className="mt-2">
                ...
                {['easy','medium','hard'].map(lv => (
                  <div key={lv} className="ml-4 mb-1">
                    ...
                    <label className="ml-2">Points: <input type="number" defaultValue={cfg.levels?.[lv]?.points||''} onChange={e => {
                      if (!cfg.levels) cfg.levels = {};
                      if (!cfg.levels[lv]) cfg.levels[lv] = {};
                      cfg.levels[lv].points = +e.target.value;
                    }} /></label>
                    <label className="ml-2">Retry Penalty: <input type="number" defaultValue={cfg.levels?.[lv]?.retryPenalty||''} onChange={e => {
                      if (!cfg.levels) cfg.levels = {};
                      if (!cfg.levels[lv]) cfg.levels[lv] = {};
                      cfg.levels[lv].retryPenalty = +e.target.value;
                    }} /></label>
                  </div>
                ))}
              </div>
            )}
            <label>Scratcher Enabled: <input type="checkbox" defaultChecked={cfg.scratcher?.enabled} onChange={e => cfg.scratcher = { ...cfg.scratcher, enabled: e.target.checked }} /></label><br />
            <label>Scratcher Drops (prob,points): <input type="text" defaultValue={JSON.stringify(cfg.scratcher?.drops || [])} onChange={e => cfg.scratcher = { ...cfg.scratcher, drops: JSON.parse(e.target.value) }} /></label><br />
      ...
      <div className="mt-8">
        ...
        <input type="text" value={gotd} onChange={e => setGotd(e.target.value)} />
